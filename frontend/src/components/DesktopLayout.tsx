...
export function DesktopLayout({
  ...
  // Add talent pool tab for companies (CVs guardados, reutilizables)
  if (isCompanyView) {
    navItems.splice(3, 0, {
  ...
  // Add team tab for the company owner (manage RRHH accounts)
            ...
            <div className="flex items-center gap-4">
              {showBack &&
                (onBack ? (
                  <button
                    onClick={onBack}
                    className="p-2 -ml-2 hover:theme-bg-card rounded-lg transition-colors"
                  >
                    <svg
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="max-w-4xl mx-auto">
            {userData?.companySubscription?.expired && (
              <div className="mb-6 p-4 rounded-xl bg-red-100 border border-red-300 text-red-800 text-sm">
                <span className="font-semibold">Tu plan venció.</span> Podés ver tu cuenta, pero
                las búsquedas, el talent pool y el análisis de CVs están deshabilitados.
                Contactá al administrador para renovarlo.
              </div>
            )}
            {children}
          </div>
